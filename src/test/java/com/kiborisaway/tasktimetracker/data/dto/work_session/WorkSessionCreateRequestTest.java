...
class WorkSessionCreateRequestTest {
  ...
  void typeにnullが渡されたときバリデーション違反になること() {
    ...
    // Act
    Set<ConstraintViolation<WorkSessionCreateRequest>> violations = validator.validate(request);
  ...
  @Test
  void TIMERでstartedAtがあるときバリデーション違反にならないこと() {
    // Arrange
    WorkSessionCreateRequest request = new WorkSessionCreateRequest();
    request.setType(WorkSessionType.TIMER);
